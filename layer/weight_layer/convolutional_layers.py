...
class ConvolutionalLayer(WeightLayer):
    def __init__(self, filter_dim, strides, padding, func=tf.nn.conv2d, use_cudnn_on_gpu=True, data_format='NHWC',
        ...
        self.func = func
        self.use_cudnn_on_gpu = use_codnn_on_gpu
    ...
    def build_forward(self, input, remember_input=False, gather_stats=False):
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            filter_shape = [self.filter_dim[0], self.filter_dim[1], input.shape[3],
                            strides[data_format.index['C']]]
            filters = tf.get_variable("filters", filter_shape,
                                      initializer=self.filters_initializer())
            self.variables.append(filters)
            output = self.func(input, filters, strides=self.strides, padding=self.padding, use_cudnn_on_gpu=self.use_codnn_on_gpu, data_format=self.data_format, dilations=self.dilations)
            if self.add_biases:
