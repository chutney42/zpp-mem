...
def batch_relu_conv(num_filters, stride=[1, 1]):
    return [BatchNormalization(), ReLu(), Conv((3, 3), number_of_filters=num_filters, stride=stride)]


def batch_relu_conv_3(num_filters, stride=[1, 1]):
    return [BatchNormalization(), ReLu(), Conv((1, 1), number_of_filters=num_filters, stride=stride),
            BatchNormalization(), ReLu(), Conv((3, 3), number_of_filters=num_filters),
            BatchNormalization(), ReLu(), Conv((1, 1), number_of_filters=num_filters * 4),
            ]
...
def build_resnet(output_size, size_seq, convolution_generator):
    ...
    residual_seq = [Conv((3, 3), number_of_filters=num_filters)]
    for _ in range(size_seq[0] - 1):
        residual_seq += convolution_generator(num_filters)

    blocks = [Block([Conv((7, 7), number_of_filters=64, stride=[2, 2]), BatchNormalization(), ReLu(),
