...
def build_resnet(output_size, size_seq, block):
    num_filters = 64

    residual_seq = [ConvolutionalLayer((3,3), number_of_filters=num_filters)]
    for _ in range(size_seq[0]-1):
        residual_seq += block(num_filters)

    blocks = [Block([ConvolutionalLayer((7, 7), number_of_filters=64, stride=[2, 2]), BatchNormalization(), ReLu(), MaxPool([3, 3], strides=[2, 2])]),
              Block([ResidualLayer(residual_seq)])]

    for seq in size_seq[1:-1]:
        num_filters *= 2
        residual_seq = block(num_filters, [2, 2])
