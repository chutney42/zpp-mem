...
tensor_allocation_regex = re.compile(
...
raw_allocation_regex = re.compile(
...
tensor_output_regex = re.compile(
...
tensor_output_regex_no_bytes = re.compile(
...
tensor_deallocation_regex = re.compile(
...
raw_deallocation_regex = re.compile(
    """allocation_id: (?P<allocation_id>\d+).*allocator_name: \"(?P<allocator_name>[^"]+)\".*""")
...
def print_memory_timeline(log, gpu_only=False, ignore_less_than_bytes=0, file="file"):
    total_memory = 0
    with open(file, "w") as f:
        for record in memory_timeline(log):
            ...
            total_memory += allocated_bytes
            f.write("%9d %120s %11d %11d %s\n" % (i, kernel_name, allocated_bytes, total_memory, allocator_type))


import matplotlib.pyplot as plt


def plot_memory_timeline(log, gpu_only=False, ignore_less_than_bytes=1000, filename="foo"):
    total_memory = 0
    timestamps = [[], []]
    data = [[], []]
    index = 0
    ...
    for record in memory_timeline(log):
        ...
        print(kernel_name)
        if "Equal" is str(kernel_name):
            index = 1

        timestamps[index].append(current_time - .00000001)
        data[index].append(total_memory)

        # if abs(int(record[2])) > 16* total_memory + 100000:
        #    continue

        total_memory += int(record[2])
        timestamps[index].append(current_time)
        data[index].append(total_memory)
        current_time += 1
